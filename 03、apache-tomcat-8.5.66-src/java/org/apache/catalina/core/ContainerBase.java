...
public abstract class ContainerBase extends LifecycleMBeanBase
    ...
    protected synchronized void startInternal() throws LifecycleException {
        ...
        Container children[] = findChildren(); //获取到StandardHost(host)数组实例
        List<Future<Void>> results = new ArrayList<>();
        for (Container child : children) {
            results.add(startStopExecutor.submit(new StartChild(child)));//将host添加到线程池中去(需要进入到StartChild的call()方法中去)
        }

        MultiThrowable multiThrowable = null;

        for (Future<Void> result : results) {  //获取线程池执行任务call()后的返回值
            try {
                result.get();
            } catch (Throwable e) {
        ...
        setState(LifecycleState.STARTING);//Context实例化通过容器声明状态周期进行驱动进行的
